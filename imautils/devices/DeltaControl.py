...
class UndulatorControl():
    ...
    def move_until_stops(self):
        ...
        try:
            ...
            error, vel, travel_time = self.check_distance_error()
    
            # triggers movement
            self.trig_move()
            _time.sleep(0.3)
            self.wait_until_stops(travel_time)
    ...
    def move_phase(self, phase=0, mirror=0, speed=2):
        ...
        self.configure_move(coup=2, mirror=mirror, rel_pos=phase, speed=speed)
        self.move_until_stops()

    def move_counterphase(self, counterphase=0, mirror=0, speed=2):
    ...
    def move_gv(self, gv=0, mirror=0, speed=2):
        """Moves cassettes in gv mode to a relative position.

        Args:
            gv (float): gv relative position change in [mm];
            speed (float): speed in [mm/s]."""
        self.configure_move(coup=4, mirror=mirror, rel_pos=gv, speed=speed)
        self.move_until_stops()

    def move_gh(self, gh=0, mirror=0, speed=2):
        """Moves cassettes in gh mode to a relative position.

        Args:
            gh (float): gh relative position change in [mm];
            speed (float): speed in [mm/s]."""
        self.configure_move(coup=5, mirror=mirror, rel_pos=gh, speed=speed)
        self.move_until_stops()

    def move_csd(self, pos=0, speed=2):
        """Moves CSD in to a relative position.

        Args:
            pos (float): CSD relative position change in [mm];
            speed (float): speed in [mm/s]."""
        self.configure_move(coup=6, rel_pos=pos, speed=speed)
        self.move_until_stops()

    def move_cse(self, pos=0, speed=2):
        """Moves CSE in to a relative position.

        Args:
            pos (float): CSE relative position change in [mm];
            speed (float): speed in [mm/s]."""
        self.configure_move(coup=7, rel_pos=pos, speed=speed)
        self.move_until_stops()

    def move_cie(self, pos=0, speed=2):
        """Moves CIE in to a relative position.

        Args:
            pos (float): CIE relative position change in [mm];
            speed (float): speed in [mm/s]."""
        self.configure_move(coup=8, rel_pos=pos, speed=speed)
        self.move_until_stops()

    def move_cid(self, pos=0, speed=2):
        """Moves CSD in to a relative position.

        Args:
            pos (float): CSD relative position change in [mm];
            speed (float): speed in [mm/s]."""
        self.configure_move(coup=9, rel_pos=pos, speed=speed)
        self.move_until_stops()
    ...
    def configure_move(self, coup=0, mirror=0, trig_type=0,
                       rel_pos=0, speed=2, acc=1000, decel=1000):
    ...
    def wait_until_stops(self, travel_time=1):
        """Holds until the undulator stops moving.
        
            Args:
                travel_time (float): time interval [s] to complete
                the movement."""
        _t0 = _time.time()
        _time.sleep(0.5)
        while(self.query_moving()):
        # while(time.time() - _t0 < travel_time + 3):
            _time.sleep(0.5)
    ...
    def check_distance_error(self):
        ...
        to move.
            ...
            Returns:
                ...
                travel_time (float): time interval [s] to complete the 
                                     movement."""
        ...
        if travel_time < 1:
            ...
            return True, vel, travel_time

        return False, vel, travel_time

    def query_moving(self):
        """Returns True if any cassette is moving, False otherwise."""
        # CSD_moving = caget(self.pv_CSD_motion_state_mon)
        # CSE_moving = caget(self.pv_CSE_motion_state_mon)
        # CIE_moving = caget(self.pv_CIE_motion_state_mon)
        # CID_moving = caget(self.pv_CID_motion_state_mon)
        #
        # moving = any([CSD_moving == 1,
        #               CSE_moving == 1,
        #               CIE_moving == 1,
        #               CID_moving == 1,])
        #
        # print(CSD_moving, CSE_moving, CIE_moving, CID_moving)  
        moving = any([caget(self.pv_CSD_motion_state_mon) == 1,
